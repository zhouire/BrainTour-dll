...
ClientManager::ClientManager(ClientType type)
{
	...
	curPacket = false;
	nextDataSize = sizeof(PacketSize);
...
void ClientManager::sendSizeData(int packet_size) {
	...
	const unsigned int s_size = sizeof(PacketSize);
	char s_data[s_size];
...
void ClientManager::update()
{
	...
	while (i < (unsigned int)data_length)
	{
		...
		if (!curPacket) {
			...
			if ((data_length - i + tempBuf.size()) < sizeof(PacketSize)) {
			...
			//curPacket switch (because we constructed a full object), and skip rest of loop
				...
				tempBuf.append(&(network_data[i]), (sizeof(PacketSize) - s));
				size.deserialize((char*)(tempBuf.data()));
				nextDataSize = size.size;
				curPacket = !curPacket;

				i += (sizeof(PacketSize) - s);
